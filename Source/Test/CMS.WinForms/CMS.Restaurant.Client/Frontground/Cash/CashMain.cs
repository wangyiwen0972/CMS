namespace CMS.Restaurant.Client.Cash
{
    ...
    using CMS.Restaurant.Client.Sales;
    ...
    public partial class CashMain : CMSForm
    {
        ...
        void amountTimer_Tick(object sender, EventArgs e)
        {
            ...
            foreach (Entrance entra in this.entrance)
            {
                ...
                if (decimal.TryParse(this.entranceController.GetSalesRevenueByEntrance(entra,DateTime.Now).Result, out currentAmount))
                {
        ...
        {
            CardClose frmColse = new CardClose();
        ...
        private void buttonPrint_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                if (!string.IsNullOrEmpty(printer1Name))
                {
                    ...
                    XmlDocument xmlDocument = ConvertTo(this.entrance);
                    printer1.ApplyTemplate(xmlDocument, string.Format(@"{0}\{1}", Environment.CurrentDirectory, @"Template\DailyReportTemplate.xslt"));

                    printer1.SetCopies(printer2Copies);
                    printer1.Print();
                }
            }
            catch (Exception ex)
        ...
        private System.Xml.XmlDocument ConvertTo(ICollection<Entrance> entranceCollection)
        {
            decimal totallyAmount = decimal.Zero;
            int totallyCount = 0;
            string operatorName = Login.FullName;

            XmlDocument doc = new XmlDocument();

            XmlElement rootElement = doc.CreateElement("entrances");

            foreach (Entrance entrance in entranceCollection)
            {
                XmlElement entranceElement = doc.CreateElement("entrance");

                string strAmount = this.entranceController.GetSalesRevenueByEntrance(entrance, DateTime.Now).Result;

                decimal amount = decimal.Zero;
                
                decimal.TryParse(strAmount, out amount);

                int count = this.entranceController.GetOrderCountByEntrance(entrance, DateTime.Now);

                totallyAmount += amount;

                totallyCount += count;

                XmlAttribute nameAttribute = doc.CreateAttribute("name");
                nameAttribute.InnerText = entrance.EnterName;
                entranceElement.Attributes.Append(nameAttribute);

                XmlAttribute amountAttribute = doc.CreateAttribute("amount");
                amountAttribute.InnerText = amount.ToString("f2");
                entranceElement.Attributes.Append(nameAttribute);

                XmlAttribute countAttribute = doc.CreateAttribute("count");
                amountAttribute.InnerText = count.ToString();
                entranceElement.Attributes.Append(amountAttribute);

                rootElement.AppendChild(entranceElement);
            }
            XmlAttribute totallyAmountElement = doc.CreateAttribute("TotallyAmount");
            totallyAmountElement.InnerText = totallyAmount.ToString("f2");
            rootElement.Attributes.Append(totallyAmountElement);

            XmlAttribute totallyCountElement = doc.CreateAttribute("TotallyOrderCount");
            totallyCountElement.InnerText = totallyCount.ToString();
            rootElement.Attributes.Append(totallyCountElement);

            XmlAttribute operatorElement = doc.CreateAttribute("operator");
            operatorElement.InnerText = operatorName;
            rootElement.Attributes.Append(operatorElement);

            XmlAttribute operatedateElement = doc.CreateAttribute("operatedate");
            operatedateElement.InnerText = DateTime.Now.ToString("yyyy-MM-dd hh:mm");
            rootElement.Attributes.Append(operatedateElement);
